...
public class BankAccount {
	...
	public void depositMoney(double depositAmount) {
		// Rejette les valeurs invalides (NaN, Infinity) et négatives
		if (depositAmount >= 0 && !Double.isNaN(depositAmount) && !Double.isInfinite(depositAmount)) {
			balance = balance + depositAmount;
		}
	}

	public boolean withdrawMoney(double withdrawAmount) {
  		boolean success; // local succes variable instead of using class attribute
		// Decision point 1: main if with 4 conditions combined with &&
		if (withdrawAmount >= 0              // Condition 1
				&& balance >= withdrawAmount       // Condition 2
				&& withdrawAmount <= withdrawLimit  // Condition 3 - CORRIGÉ: <= au lieu de <
				&& withdrawAmount + amountWithdrawn <= withdrawLimit // Condition 4
				&& !Double.isNaN(withdrawAmount)   // Condition 5 - Rejette NaN
				&& !Double.isInfinite(withdrawAmount)) { // Condition 6 - Rejette Infinity
			balance = balance - withdrawAmount;
			success = true;
			amountWithdrawn += withdrawAmount;
			// Implicit decision point: else
		} else {
			success = false;
		}
		return success;
	...
	public int loadFromText(String text) {
		...
		try {
			fis = new FileInputStream(text); // change the location of theses 2 instructions
